...
\begin{enumerate}[leftmargin=*,itemsep=0.2em]
  ...
  \item \textbf{扩散系数的温度激活。} $D$ 含 Arrhenius 因子 $\mathrm{e}^{-3850/T_K}$，$\dd\ln D/\dd T=E/T_K^2$。在同一含水率 $C_0$ 下，温度由 \SI{28}{\celsius} 升到恒温阶段的平台值使 $D$ 由 \sci{\valQtwoDinit} 增大到 \sci{\valDmaxAppendixThree} \si{\square\meter\per\second}，即增大一倍以上。因此预热阶段虽然失水不多，却通过升温为后续干燥“解锁”了扩散能力。
...
判据在 $t_{\mathrm{end}}$ 处恰好取等号，因此结束时刻附近的中心水分浓度与 $C^\ast$ 的差远小于四位小数的分辨率：\nolinkurl{result3.xlsx} 末行（$\valQthreeDryHoursGrid$ h）的中心水分浓度精确值为 $\valQthreeCentreMoistGridRow$、\nolinkurl{result4.xlsx} 末行（$\valQfourDryHoursGrid$ h）为 $\valQfourCentreMoistGridRow$，按题目要求四舍五入到四位小数后\textbf{显示}为 $0.1500$。表~\ref{tab:q3-moist}、表~\ref{tab:q4-moist} 末行同理。判据是否成立由未舍入的解按\eqref{eq:criterion} 判定，并由与求解器分离的校验器独立确认（第~\ref{sec:validation} 节）：$t<t_{\mathrm{end}}$ 的全部输出行满足 $\max_iC_i\ge C^\ast$，$t\ge t_{\mathrm{end}}$ 的全部行满足 $\max_iC_i<C^\ast$。
\end{remark}
