...
\textbf{模型检验}（校验器与求解器代码分离）：与常物性 Robin 圆柱贝塞尔级数解的最大误差为 $\valAnalyticErrTemp$（温度）与 $\valAnalyticErrMoist$（水分），观测阶 $\valAnalyticOrderTemp$；六级网格自收敛的观测阶为 $\valConvOrderQthreeMoist$--$\valConvOrderQoneMoist$，烘干时间与最细网格之差 $\valConvDryDiffQthreeMin$ min（问题 3）、$\valConvDryDiffQfourMin$ min（问题 4）；离散水分与能量平衡是半离散系统的精确线性不变量（相对残差 $\sim10^{-15}$）；极值原理与径向单调性全程成立；更严格容差、Radau 与限制最大步长的解相差不超过 $\valTimeRadauDiffMoist$。独立编写的二维轴对称程序在端面绝热时与一维解相差 $\valCrossCheckDiffMoist$，端部效应对烘干时间的影响仅 $\valEndEffectDryDiffPct\%$，支持一维化。灵敏度分析给出烘干时间对烘房温度平台、扩散前因子与收缩幅度的弹性为 $\valElasTplateau$、$\valElasDzero$、$\valElasShrink$，而对换热系数几乎不敏感（$\valElasH$）——烘干由内部扩散而非外部传热控制。模型评价一节进一步以湿球能量上限与平衡含水率驱动力量化了题给模型省略的蒸发潜热与吸附平衡两项机制的影响范围。
