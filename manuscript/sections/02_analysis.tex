...
\begin{enumerate}[leftmargin=*,itemsep=0.25em]
  ...
  \item \textbf{"各处低于 0.15"是一个停止时刻问题。} 判据 $\max_r C(r,t)<0.15$ 定义的时刻不是输出网格上的点。若只在 \SI{60}{\second} 网格上判断，分辨率误差可达 \SI{60}{\second}；对策是把 $g(t)=\max_i C_i(t)-0.15$ 作为积分器的事件函数精确求根，并同时报告 \SI{60}{\second} 网格上的值以与结果文件对照。
  ...
  \item \textbf{四位小数的精度要求。} 表格与结果文件保留四位小数，要求离散误差远小于 $5\times10^{-5}$。对策是把输出点取在网格节点上（$\Delta r=0.1/2^k$ \si{\centi\meter}），用收敛表确定 $k$，并用解析级数解独立确认误差量级（第~\ref{sec:validation} 节）。
\end{enumerate}
