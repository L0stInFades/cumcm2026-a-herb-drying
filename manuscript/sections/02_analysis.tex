...
\begin{enumerate}[leftmargin=*,itemsep=0.25em]
  ...
  \item \textbf{扩散系数跨越三到四个数量级。} $D\propto\mathrm{e}^{-a/C}$ 在干燥后期急剧变小：以附录 3 计，$D$ 由初始（\SI{28}{\celsius}、$C_0$）的 \sci{\valQtwoDinit} \si{\square\meter\per\second} 先随升温增大到过程最大值 \sci{\valDmaxAppendixThree} \si{\square\meter\per\second}（恒温平台、$C_0$），再降到烘干结束时表面的 \sci{\valDsurfaceAtDryQthree} \si{\square\meter\per\second}，最大值与末值之比为 $\valDratioQthree$（图~\ref{fig:properties}）。表面因此形成低扩散率的“硬壳”，同一参数体系下的传质 Biot 数 $h_mR_0/D$ 由初始的 $\valQtwoBiotMassInit$ 增至约 $\valBiotMassAtDryQthree$，过程由\textbf{内部扩散}而非外部对流控制——这一判断在灵敏度分析中被定量证实（弹性 $\valElasH$ 对 $h$、$\valElasDzero$ 对 $D_0$）。
  ...
  \item \textbf{问题 4 是移动边界问题。} 计算域随 $R(t)$ 收缩，而附件 2 只给几何、未给物理原则。本文以干物质守恒的物质坐标（Lagrangian）为主模型，实验室坐标写法作为定量对照（第~\ref{subsec:q4model} 小节）。
  \item \textbf{四位小数的精度要求。} 离散误差必须远小于 $5\times10^{-5}$。对策是把输出点取在网格节点上（$\Delta r=0.1/2^k$ \si{\centi\meter}），并在结果文件的\textbf{完整}输出网格上逐格评估误差（第~\ref{subsec:gridconv} 小节）。
\end{enumerate}
...
\node[box,below=9mm of data.south west,anchor=north west,text width=23mm] (q1) {问题~1\\\footnotesize 附录 2};
\node[box,right=4mm of q1,text width=23mm] (q2) {问题~2\\\footnotesize 附录 3};
\node[box,right=4mm of q2,text width=23mm] (q3) {问题~3\\\footnotesize 判据 0.15};
\node[box,right=4mm of q3,text width=25mm] (q4) {问题~4\\\footnotesize 附录 4 $+$ Landau};
...
\draw[ar] (data) -- (pde);
\draw[ar] (pde) -- (num);
\coordinate (bus) at ($(num.south)+(0,-4mm)$);
\draw[draw=ink!70,thick] (num.south) -- (bus);
\draw[draw=ink!70,thick] (q1.north|-bus) -- (q4.north|-bus);
\draw[ar] (q1.north|-bus) -- (q1.north);
\draw[ar] (q2.north|-bus) -- (q2.north);
\draw[ar] (q3.north|-bus) -- (q3.north);
\draw[ar] (q4.north|-bus) -- (q4.north);
...
\end{tikzpicture}
\caption{技术路线：由守恒律建立统一的一维径向热质耦合初边值问题，四个问题从\textbf{同一条}离散与积分总线分出，只改变物性、时间范围与边界是否移动；每一组解都经与求解器分离的独立校验，并给出灵敏度与备选解释。}
\label{fig:route}
\end{figure}
