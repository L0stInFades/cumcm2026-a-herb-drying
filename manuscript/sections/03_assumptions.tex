...
$\bar C$ & 体积平均水分浓度 & \si{\kilo\gram\per\kilo\gram} & $\bar C=\frac{2}{R^2}\int_0^R\!C r\,\mathrm{d}r$ \\
...
$\alpha$ & 热扩散率 $k/(\rho c_p)$ & \si{\square\meter\per\second} & $\valQoneAlpha$（附录 2） \\
...
$\mathrm{Fo},\mathrm{Fo}_m$ & 热、质 Fourier 数 $\alpha t/R_0^2$、$Dt/R_0^2$ & --- & 见第~\ref{subsec:q1analysis} 小节 \\
$C^\ast$ & 烘干结束的水分浓度阈值 & \si{\kilo\gram\per\kilo\gram} & $0.15$ \\
$t_{\mathrm{end}}$ & 烘干结束时间 & \si{\second}（报告为 h） & 求解结果 \\
$n,\Delta\xi$ & 径向控制体个数、无量纲步长 $1/n$ & --- & $n=\valGridNodes-1$ \\
