...
def derived(ctx: StageContext) -> dict[str, Any]:
    ...
    for prob, tag in (("q1", "Qone"), ("q2", "Qtwo"), ("q3", "Qthree"), ("q4", "Qfour")):
        sampled = ctx.dep_data(prob, f"verification_{prob}.json")["moisture_balance"].get("sampled")
        if sampled is not None:
            ctx.number(f"SampledResidual{tag}", sampled["max_relative_residual"], ".1e")
    last3 = float(np.asarray(q3h["t"], dtype=float)[-1])
    last4 = float(np.asarray(q4h["t"], dtype=float)[-1])
    ctx.number("QthreeLastRowHours", last3 / HOUR, ".2f")
    ctx.number("QfourLastRowHours", last4 / HOUR, ".2f")
    ctx.number("QthreeLastRowExtraMin", (last3 - t3) / 60.0, ".1f")
    ctx.number("QfourLastRowExtraMin", (last4 - float(q4["main"]["t_dry_s"])) / 60.0, ".1f")
    ctx.number("QfourRadiusHalfHourCm", value_at(q4h["t"], q4h["radius_cm"], 1800.0), ".3f")
    q1m = pd.read_parquet(ctx.dep("q1") / "profiles_moist.parquet")
    one_cm_end = float(q1m["1.0"].to_numpy(float)[-1])
    ctx.number("QoneMoistOneCmEnd", one_cm_end)
    ctx.number("QoneMoistOneCmDropPct", 100.0 * (1.0 - one_cm_end / C_INIT), ".1f")
    ctx.number("QtwoRhoSWet", float(dry_solid_density(APPENDIX3, np.array(C_INIT))), ".0f")
    ctx.number("QtwoRhoSDry", float(dry_solid_density(APPENDIX3, np.array(C_TARGET))), ".0f")
