...
MEMBER_DESCRIPTIONS = (
    ...
    (r"^pipelines/a/physics\.py$", "题给物性与经验公式（附录 2/3/4）"),
    (r"^pipelines/a/solver\.py$", "径向有限体积离散与隐式时间积分"),
    (r"^pipelines/a/science\.py$", "四个问题的求解编排与输出采样"),
    (r"^pipelines/a/verify\.py$", "与求解器分离的独立校验（解析解、守恒、收敛阶）"),
    (r"^pipelines/a/derived\.py$", "论文派生量的集中计算"),
    (r"^pipelines/a/outputs\.py$", "结果工作簿的生成"),
    (r"^pipelines/a/figures\.py$", "论文图与表的生成"),
    (r"^pipelines/a/stages\.py$", "本题各阶段的编排"),
